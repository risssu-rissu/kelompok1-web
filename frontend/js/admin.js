...
async function loadStatistics() {
    try {
        ...
        if (result.success) {
            ...
            const recent = [...students].slice(0, 5); // Already sorted by createdAt DESC in controller
            if(recent.length > 0) {
...
function renderMasterTable() {
    ...
    tbody.innerHTML = result.map(student => `
        <tr>
            ...
            <td>
                <button class="btn btn-info btn-sm" onclick="viewStudent('${student._id}')">Lihat</button>
                <button class="btn btn-success btn-sm" onclick="editStudent('${student._id}')">Edit</button>
                <button class="btn btn-danger btn-sm" onclick="deleteStudent('${student._id}')">Hapus</button>
            </td>
        </tr>
    `).join('');
}
...
// Display students in table (Specific Class View)
...
function displayClassCards(classes) {
    ...
    if (classes.length === 0) {
        container.innerHTML = '<p style="text-align: center; padding: 40px; color: var(--gray-400);">Belum ada kelas tersedia. Tambahkan siswa pertama kali untuk membuat kelas otomatis.</p>';
        return;
    }
...
// Open add modal
let originalOpenAddModal = function() {
    currentEditId = null;
    ...
    document.getElementById('statusKeaktifan').value = 'Aktif';
    document.getElementById('studentModal').classList.add('show');
}
window.openAddModal = originalOpenAddModal;

// Open add modal for selected class
function openAddModalForClass() {
    currentEditId = null;
    document.getElementById('modalTitle').textContent = `Tambah Siswa - ${selectedClass}`;
    document.getElementById('studentForm').reset();
    document.getElementById('studentId').value = '';
    document.getElementById('kelas').value = selectedClass;
...
function editStudent(id) {
    ...
    document.getElementById('kelas').readOnly = false; 
    document.getElementById('tanggalLahir').value = formatDateForInput(student.tanggalLahir);
    document.getElementById('noTelepon').value = student.noTelepon;
    document.getElementById('alamat').value = student.alamat;
    document.getElementById('emailStudent').value = student.email || '';
    document.getElementById('statusKeaktifan').value = student.statusKeaktifan || 'Aktif';
...
async function deleteStudent(id) {
    const result_swal = await Swal.fire({
        title: 'Hapus Data?',
        text: 'Apakah Anda yakin ingin menghapus data siswa ini secara permanen? Tip: Jika siswa lulus/pindah, Anda dapat Edit dan mengubah Status Keaktifan-nya.',
        icon: 'warning',
    ...
    try {
        ...
        if (result.success) {
            ...
            // Refresh specifics if we are in class view
            if (selectedClass) {
...
document.getElementById('studentForm').addEventListener('submit', async (e) => {
    ...
    const formData = {
        ...
        statusKeaktifan: document.getElementById('statusKeaktifan').value
    };
    ...
    try {
        ...
        if (result.success) {
            closeModal();
            // Reload all global datasets
            await loadStudents();
            await loadClasses();
            loadStatistics();
            
            if (selectedClass && document.getElementById('studentListView').style.display !== 'none') {
                selectClass(selectedClass);
            } else {
                loadClassCards();
            }
...
function exportExcel() {
    ...
    if(result.length === 0){
        Swal.fire('Info', 'Tidak ada data untuk dieksport', 'info'); return;
    }
...
async function processExcelImport() {
    ...
    if (!fileInput.files.length) {
        Swal.fire('Perhatian', 'Silakan pilih file terlebih dahulu', 'warning');
        return;
    }
    ...
    reader.onload = async function(e) {
        try {
            ...
            const json = XLSX.utils.sheet_to_json(worksheet, { defval: "" }); // preserve empty strings
            
            if (json.length === 0) {
                Swal.fire('Perhatian', 'File kosong atau format salah.', 'warning'); return;
            }

            // Normalisasi key
            const payload = json.map(row => {
                // Mencari key case-insensitive
                const findKey = (k) => Object.keys(row).find(key => key.toLowerCase() === k.toLowerCase()) || null;
                return {
                    nisn: row[findKey('nisn')]?.toString() || '',
                    nama: row[findKey('nama')] || row[findKey('nama lengkap')] || '',
                    jenisKelamin: row[findKey('jeniskelamin')] || row[findKey('jenis kelamin')] || '',
                    kelas: row[findKey('kelas')]?.toString() || '',
                    alamat: row[findKey('alamat')] || '-',
                    tanggalLahir: row[findKey('tanggallahir')] || row[findKey('tanggal lahir')] || '2000-01-01',
                    statusKeaktifan: row[findKey('statuskeaktifan')] || row[findKey('status')] || 'Aktif'
                }
            });
            ...
            if (result.success) {
                if(result.errors && result.errors.length > 0) {
                    Swal.fire('Selesai dengan Catatan', `Berhasil: ${result.imported}\nGagal: ${result.failed}\n\nContoh error:\n${result.errors.slice(0,3).join('\n')}`, 'warning');
                } else {
                    Swal.fire('Sukses!', `${result.imported} data siswa berhasil di import.`, 'success');
                }
                closeImportModal();
                loadStudents();
                loadClasses();
                ...
                Swal.fire('Gagal!', `Gagal upload batch: ${result.message}`, 'error');
            }

        } catch(err) {
            console.error(err);
            Swal.fire('Gagal!', 'Gagal membaca atau memproses file excel.', 'error');
        }
    };
